...
Runtime requirements are hyprlock 0.9.6 or newer, PAM, Inter, Rajdhani, and JetBrainsMono Nerd Font. NetworkManager's
`nmcli` and WirePlumber's `wpctl` are optional: their indicators are hidden when unavailable. Battery state is read
from Linux sysfs.
...
project (`qml/images/no-avatar.png`) and retains compatible provenance.
